...
from anicli_api.tools import generate_playlist, generate_asyncio_playlist
...
async def test_async_generate_m3u():
    ...
    assert (await generate_asyncio_playlist(target=["1.mp4", "2.mp4", "3.mp4"], names=["v1", "v2", "v3"])
            ) == RESULT_WITH_NAMES
