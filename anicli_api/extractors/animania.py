...
class Extractor(BaseAnimeExtractor):
    ...
    def _ongoing_parse(self, response: str):
        divs_ksupdate = self._soup(response).find_all("div", class_="ksupdate_block_list_item_inner")
        ongs = []
        ...
        for ul in soup.find("ul", id="ks-episodes"):
            for span in ul.find_all("span"):
                data = {
                    ...
                    "url": "https:" + span["onclick"].replace("kodikSlider.player('", "").replace("', this);", "")
                }
...
class TestCollections(BaseTestCollections):
    ...
    def test_extract_metadata(self):
        for meta in Extractor().search("serial experiments lain")[0]:
            assert meta.search.url == "https://animania.online/9403-jeksperimenty-ljejn-serial-experiments-lain-1998-smotret-onlajn.html"
