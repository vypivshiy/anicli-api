...
class Extractor(BaseExtractor):
    ...
    def ongoing(self) -> List["Ongoing"]:
        ...
        return Ongoing.from_crop_rule_list(response.text,
                                           crop_rule=cbxa('//*[starts-with(@class, "last-update-item")]'))
...
class Search(BaseSearch):
    ...
    rating: ScField[float, ParselXPath("//div[@class='p-rate-flag__text']", default="0",
                                       callback=replace_text(",", "."))]
    name: ScField[str, ParselXPath(
        "//div[@class='text-gray-dark-6 small mb-1 d-none d-sm-block']/div")]
    title: ScField[str, ParselXPath(
...
class Ongoing(BaseOngoing):
    _thumb_style: ScField[str, ParselXPath("//div[@class='img-square lazy br-50']",
                                           callback=get_attr("style"))]
    ...
    dub: ScField[str, ParselXPath("//div[@class='ml-3 text-right']/div[@class='text-gray-dark-6']")]
...
class Anime(BaseAnime):
    rating: ScField[float, ParselXPath('//*[@id="itemRatingBlock"]/div[1]/div[2]/div[1]/span[1]',
                                       default="0", callback=replace_text(',', '.'))]
    ...
    _raw_metadata: ScField[str, ParselXPath('//div[@class="anime-info"]/dl[@class="row"]',
    ...
    description: ScField[str,
    ParselXPath(
        "//div[@class='description pb-3']", callback=get_text(strip=True, sep=" ")),  # mobile agent
    ParselXPath(
        "//div[@data-readmore='content']", callback=get_text(strip=True, sep=" "))]  # desktop agent
    ...
    def get_episodes(self) -> List["Episode"]:
        ...
        return Episode.from_crop_rule_list(response, crop_rule=cbxa('//*[@id="video-carousel"]/div/div'),
...
class Episode(BaseEpisode):
    ...
    def get_sources(self) -> List["Source"]:
        response = self.HTTP().get('https://animego.org/anime/series',
                                   params={"dubbing": 2,
                                           "provider": 24,
                                           "episode": self.num,
                                           "id": self.data_id}).json()["content"]
        return Source.from_crop_rule_list(response, crop_rule=cbxa('//*[@id="video-players"]/span', ),
                                          _dubbers_table=self._dubbers_table)

    async def a_get_sources(self) -> List["Source"]:
        async with self.HTTP_ASYNC() as client:
            response = await client.get('https://animego.org/anime/series',
                                        params={"dubbing": 2,
                                                "provider": 24,
                                                "episode": self.num,
                                                "id": self.data_id}).json()["content"]
...
class Source(BaseSource):
    ...
    _data_provide_dubbing: ScField[str, ParselXPath("//span", callback=get_attr('data-provide-dubbing'))]
